...
impl Oracle {
    ...
    fn initialize_plain_text(&mut self) {
        self.plain_text_buffer = b"A".repeat(
            self.unknown_string_size
                .expect("Approximate string size not calculated"),
        );
    }
    ...
    pub fn decipher(&mut self) {
        ...
        for i in 1..unknown_string_size {
            ...
            let encrypted_target =
                self.encryption_oracle_with_offset(&self.plain_text_buffer[0..n]);
    ...
    pub fn detect_block_size(&self) -> Option<usize> {
        ...
        let encrypted_iter = std::iter::repeat_with(|| {
            a.push(b'A');
            self.encryption_oracle_with_offset(&a)
        });
    ...
    pub fn detect_prefix_size(&self) -> Option<usize> {
        ...
        for i in 0..AES_BLOCK_SIZE {
            ...
            if let Some(consecutive_index) =
                oracle_cipher.detect_consecutive_repetitions(AES_BLOCK_SIZE, repetitions)
            {
