...
impl Pkcs7Padding for [u8] {
    ...
    fn validate_padding(&self) -> Result<&Self> {
        ...
        if padding_size <= self.len() {
            ...
            if self.iter().skip(padding_start).all(|&c| c == padding) {
...
impl Pkcs7Padding for str {
    ...
    fn validate_padding(&self) -> Result<&str> {
        self.as_bytes().validate_padding().map(|s| s.to_str())
    }
}
...
mod tests {
    ...
    fn test_padding_with() {
        ...
        assert_eq!(
            "YELLOW SUBMARINE".with_padding(20),
    ...
    fn test_padding_without() {
        // &str
        assert_eq!(
            "YELLOW SUBMARINE\x04\x04\x04\x04".without_padding(),
            "YELLOW SUBMARINE"
        );
        assert_eq!("HEJHEJHEJHEJHEJ\x01".without_padding(), "HEJHEJHEJHEJHEJ");
        assert_eq!(
            "YELLOW\x04 SUBMARINE\x04\x04\x04\x04".without_padding(),
            "YELLOW\x04 SUBMARINE"
        );
        assert_eq!("HEJHEJHEJHEJHEJ".without_padding(), "HEJHEJHEJHEJHEJ");
        assert_eq!(
            "YELLOW SUBMARINE\x01\x02\x03\x04".without_padding(),
            "YELLOW SUBMARINE\x01\x02\x03\x04"
        );
    }
    ...
    fn test_padding_validate() {
        ...
        assert_eq!(
            "ICE ICE BAB\x05\x05\x05\x05\x05"
                .as_bytes()
                .validate_padding()
                .unwrap(),
            "ICE ICE BAB".as_bytes()
        );
        assert_eq!(
            "YELLOW SUBMARIN\x01".as_bytes().validate_padding().unwrap(),
            "YELLOW SUBMARIN".as_bytes()
        );
        assert_eq!(
        ...
        assert!("ICE ICE BABY\x01\x02\x03\x04"
            ...
            .validate_padding()
            .is_err());
        assert!("YELLOW SUBMA\x01\x02\x03\x04"
            .as_bytes()
            .validate_padding()
            .is_err());
    }
