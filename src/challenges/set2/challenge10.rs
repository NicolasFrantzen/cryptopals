...
{
    fn decrypt_aes_128_cbc(&self, key: &[u8], iv: Option<&[u8]>) -> Vec<u8>;
    fn encrypt_aes_128_cbc(&self, key: &[u8], iv: Option<&[u8]>) -> Vec<u8>;
}
...
impl Aes128CbcT for [u8]
{
    fn decrypt_aes_128_cbc(&self, key: &[u8], iv: Option<&[u8]>) -> Vec<u8>
    {
        decrypt_aes_128_cbc(self, key, iv)
    }

    fn encrypt_aes_128_cbc(&self, key: &[u8], iv: Option<&[u8]>) -> Vec<u8>
    {
        encrypt_aes_128_cbc(self, key, iv)
    }
}


fn decrypt_aes_128_cbc(cipher_buffer: &[u8], key: &[u8], iv: Option<&[u8]>) -> Vec<u8>
{
...
fn encrypt_aes_128_cbc(plain_buffer: &[u8], key: &[u8], iv: Option<&[u8]>) -> Vec<u8>
{
    let cipher = Cipher::aes_128_ecb();
    let block_size = cipher.block_size();
...
mod tests
{
    ...
    fn test_challenge10_decrypt()
    {
        ...
        let plain_text = cipher_buffer.decrypt_aes_128_cbc(key, None).to_string();
    ...
    fn test_challenge10_encrypt()
    {
        let key = "YELLOW SUBMARINE".as_bytes();
        let cipher_text = read_to_string("data/10.txt").expect("Unable to read file.");
        let cipher_buffer = "I'm back and I'm ringin' the bell".as_bytes().encrypt_aes_128_cbc(key, None);
